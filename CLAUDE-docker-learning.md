...
- **Stages 0, 1, and 2 complete.** (0 and 1 finished 2026-07-14; 2 finished 2026-07-16.) Working across **two PCs** — the original machine plus a second one (Windows 11).
- Image `mcr.microsoft.com/mssql/server:2022-latest` pulled on both machines (re-pulled on the new PC 2026-07-14, ~1.5 GB).
- **Stage 2:** wrote a single-stage `Dockerfile` by hand, built `marketer-api:naive` (**2.08 GB** — SDK base ships compilers). Ran it; boots in **Production** (no `ASPNETCORE_ENVIRONMENT` set), so dev-gated `Migrate()` is skipped and it does NOT crash at boot — the DB failure is **lazy**: `/healthz` returns **503** because the DbContext probe can't reach `(localdb)\mssqllocaldb` (Windows-only; absent in a Linux container). Fixed the guide's stale "it will crash" claim to match. Stage 4 solves it.
...
## Stage 2 — Naive Dockerfile for the API `[x]`
...
- `[x]` First build **without** a `.dockerignore`: `docker build -t marketer-api:naive .` — note the "transferring context" size in the first output lines (it ships `bin/`, `obj/`, `.git/` — huge). Then create `.dockerignore` (`**/bin`, `**/obj`, `.git/`) and rebuild — compare context size. *(2026-07-16: built WITHOUT `.dockerignore` → 2.08 GB image. 2026-08-13: `.dockerignore` added at repo root — `**/bin`, `**/obj`, `.git`, `.vs`, `CLAUDE*`, `.env`, `docs` — and reviewed; `CLAUDE*` + `.env` exclusions enforce the standing no-secrets/no-knowledge-files rule. Rebuild context-size comparison not captured.)*
- `[x]` Read every line of the build output — each `RUN`/`COPY` = one **layer**.
- `[x]` `docker image ls` — write the naive size here: 2.08 GB (expect ~1 GB+, because the SDK image ships compilers you don't need at runtime).
- `[x]` Run it: `docker run --rm -p 8080:8080 -e ASPNETCORE_URLS=http://+:8080 marketer-api:naive`. It does **not** crash at boot: no `ASPNETCORE_ENVIRONMENT` is set, so it defaults to **Production**, and `Migrate()` is dev-gated (`Program.cs` runs it only in Development) — so the DB is never touched at startup. The failure is **lazy**: hit `/healthz` from a second terminal → **503**, because the `AddDbContextCheck` probe can't reach `(localdb)\mssqllocaldb`, which doesn't exist inside a Linux container (LocalDB is Windows-only). *(To see the boot-time crash instead, add `-e ASPNETCORE_ENVIRONMENT=Development` so `Migrate()` runs and blows up immediately.)* That's Stage 4's problem to solve.
- `[x]` Self-quiz: what exactly is "the build context"? Why does `.dockerignore` matter? What is a layer?
...
| 2026-07-16 | **2 — complete** ✅ | Naive Dockerfile, written by hand. Aha-moments: **a Dockerfile is instructions-only — no English words** (`FROM` takes exactly the image name; `FROM the .NET SDK image mcr...` = "requires one or three arguments"). **An image name is an address** (`registry / repo : tag`), not glued-together words. **`RUN` = build time, `ENTRYPOINT` = run time** — SDK base needed because compilers run in `RUN`. **Running an image = starting its ENTRYPOINT process**; a web server never exits, so the container "gives no answer" = it's serving (that's success, not a hang). Naive image **2.08 GB**. DB failure is **lazy** not boot-time: Production skips `Migrate()`, `/healthz` → **503** (DbContext probe can't reach LocalDB, which is Windows-only). **Layer** = filesystem diff from one Dockerfile line; image = stacked layers; each cached. Quiz: build-context and `.dockerignore` corrected (context = the source-folder tree sent to the engine, not dlls; `.dockerignore` shrinks that context), layer taught from scratch. Also (off-track): pinned transitive `Microsoft.OpenApi` 2.11.0 to clear GHSA-v5pm-xwqc-g5wc via `CentralPackageTransitivePinningEnabled`. |
