...
- `[~]` First build **without** a `.dockerignore`: `docker build -t marketer-api:naive .` — note the "transferring context" size in the first output lines (it ships `bin/`, `obj/`, `.git/` — huge). Then create `.dockerignore` (`**/bin`, `**/obj`, `.git/`) and rebuild — compare context size. *(2026-07-16: built WITHOUT `.dockerignore` → 2.08 GB image. The `.dockerignore` create-and-compare half is still TODO — no file exists yet.)*
